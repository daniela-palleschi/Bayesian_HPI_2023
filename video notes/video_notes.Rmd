...
- *f* = *PDF*: probability density function (continuous)
  - so $f(\theta|y)$: PDF of theta given a certain value of y
...
- $P(A|B) = \frac{P(A,B)}{P(B)}$ where $P(B) > 0$
- *P(A,B) = P(B,A)*
...
- but in real world we're usually working with multivariate (and continuous) data
  ...
  - $\theta$ is now a random variable: *f($\theta$)
  - $f(\theta|y)$: PDF of $\theta$ given our observed data (y) 
  
- an example with a discrete RV: if *n* = 10 trials and *k* = 7 successes, and if we suppose there are three possible values of $\theta$: .1,.5, and .9 and each has the probability of 1/3, the likelihood function would be:
...
- an analytical example with the Poisson-gamma:
  + $\frac{a*}{b*}$: the posterior mean
  + $a*$ = $a + \sum{x_i}$
  + $b*$ = $n + b$
  + $a$: successes
  + $b$: failures
...
$$
\begin{align}
\frac{a*}{b*} &= \frac{a + \sum_{x_i}}{n+b}\\
\end{align}
$$

- we can swap out $\sum_{x_i}$:
  + $\sum_{x_i}$: sum of all observations
  + $\sum_{x_i} = n\bar{x}$: the sum of all observations equals n times the mean
  + $\bar{x}$: mean
...
- and divide the numerator and denominator by $b$
  + $(x+y)/y = 1 + x/y$
    + e.g.,: $(30+6)/6 = 36/6 = 6$, and $1 + (30/6) = 1 + 5 = 6$ 

$$
\begin{align}
\frac{a*}{b*} &= \frac{(a + n\bar{x})/b}{(n+b)/b}\\
&= \frac{\frac{a}{b} + \frac{n\bar{x}}{b}}{1+ \frac{n}{b}}\\
\end{align}
$$

  + $\frac{a}{b}$: prior mean, so we can rewrite the equation as
  
$$
\begin{align}
\frac{a*}{b*} &= \frac{m + \frac{n\bar{x}}{b}}{1+ \frac{n}{b}}\\
\end{align}
$$

- and then we can break up the numerator to be re-written as:

$$
\begin{align}
\frac{a*}{b*} &= \frac{m + \frac{n}{b}\bar{x}} {1+ \frac{n}{b}}\\
              &= \frac{m} {1+ \frac{n}{b}} +     
              \frac{\frac{n}{b}\bar{x}} {1+ \frac{n}{b}}\\
\end{align}
$$

- this is now a weighted average:
  + setting $w_1 = 1$ and $w_2 = \frac{n}{b}$, we can write the above as:
...
- this is also true for the variance of the posterior
  + as $n$ approaches 0 the posterior variance (uncertainty) will reduce

$$
\frac{a*}{b*^2} = \frac{{a + n\bar{x}}}{(n+b)^2}
$$

- so getting a lot of data will get us a tight posterior
...
Next steps: realistic data analysis
- with real daa, we can't use these simple conjugate analyses
- for such cases, we need to use MCMC (Markov chain Monte Carlo) sampling etchniques so that we can sample from the posterior distributions of the parameters
...
- we see a mean effect of cloze probability of `r round(mean(as_draws_df(fit_N400_v)$b_c_cloze),2)` 95\% CrI [`r round(quantile(as_draws_df(fit_N400_v)$b_c_cloze, c(.025,.975)),3)`]. The positive slope indicates that the larger the cloze probability, the larger the N400...counter intuitive? Should be the other way around I'd expect (negative correlation?).
...
```{r, echo = F, cache = F}
# beep to let me know the script has run
beepr::beep()
```