...
- our estimates don't change much in comparison to the varying intercept/slope model, probably becasue the estimation of the correlation is quite poor (i.e., there's a lot of uncertainty, see the plot and/or model summary below)
...
- we're almost at the "maximal" hierarchical model [@barr_random_2013-2]
  + i.e., model with all by-participant and -item group-level variance components allowed by the experimental design and a full variance covariance matrix for all the group-level paramters
  + not all variance components are allowed by the design: between-group manipulations cannot have variance components

### By-subjects and by-items correlated varying intercept varying slopes model ($M_{sih}$)

- we will now allow for differences in intercepts (mean voltage) *and* slopes (effects of predictability) across subjects and across items
- in typical Latin square designs, subjects and items are said to be *crossed random effects*: each subject sees exactly one instance of each item
  + we assume a possible correlation between varying intercepts and slopes by subjects, and another by items
...
2. Some aspects of the mean signal voltage and of the effect of predictability depend on the subject, i.e., we assume group-level intercepts and slopes and a correlation between them by-participant
3. same as above for item: some dependence in observed effects and mean voltage based on item
...
- where the index $j$ represents each item
...
- we set identical priors for by-items group-level effects as for by-subject level effects because we don't have any differentiated prior information about subject-level vs. item-level variation
...
- but we can also simplify the call to `brms` when assigning the same priors to the by-subject and -item parameters:
...
- again, the estimate of the effect of cloze remains pretty much unchanged
  + the correlation parameter for both by-participant and -item intercepts and slopes have a lot of uncertainty 

### Beyond the maximal model–Distributional regression models

- we can look at how our model captures the signal in our data overall 
...
- looks like some subjects aren't fitted very well
